  ...
  _createClass(Theme, [{
    ...
    value: function sortable() {
      if (window.Sortable) {
          ...
          options.animation = options.animation || 150;
            ...
            options.store = {
		        set: function (sortable) {
                    ...
                    console.log("Sortable", order, options.group)
                    if (options.group == "board") {
                        $.ajax({
                            url: "/giaidoan/UpdateOrderGiaiDoan",
                            type: 'POST',
                            data: { giaiDoanIds: order },
                        ...
                        $.ajax({
                            url: "/buocthuchien/UpdateOrderBuocThucHien",
                            type: 'POST',
                            data: { buocThucHienIds: order },
                            success: function (response) {
                                if (response.Status) {
                                    toastr.success(response.Message);
                                } else {
        ...
        if (window.Sortable) {
            $('[data-toggle="sortableCongviec"]').each(function () {
                var selector = this;
                var options = $(selector).data();
                options.animation = options.animation || 150;
                options.filter = options.filter || '.ignore-sort';
                options.store = {
                    set: function (sortable) {
                        var order = sortable.toArray();
                        $.ajax({
                            url: "/congviec/UpdateOrderCongViec",
                            type: 'POST',
                            data: { congViecIds: order },
                            success: function (response) {
                                if (response.Status) {
                                    toastr.success(response.Message);
                                } else {
